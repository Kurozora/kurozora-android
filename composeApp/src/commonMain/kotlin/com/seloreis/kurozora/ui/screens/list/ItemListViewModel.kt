package com.seloreis.kurozora.ui.screens.list
    ...
    fun fetchItemDetail(itemId: String, type: ItemType) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val item: Any? = when (type) {
                ...
                ItemType.Season -> (kurozoraKit.season().getDetails(itemId) as? Result.Success)?.data?.data?.firstOrNull()
                ItemType.Studio -> (kurozoraKit.studio().getStudio(itemId) as? Result.Success)?.data?.data?.firstOrNull()
                ItemType.Cast -> (kurozoraKit.cast().getCast(itemId) as? Result.Success)?.data?.data?.firstOrNull()
                ItemType.User -> (kurozoraKit.auth().getUserProfile(itemId) as? Result.Success)?.data?.data?.firstOrNull()
                else -> null
            }
